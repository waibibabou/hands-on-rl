...
class ReplayBuffer:
    ...
    def __init__(self, capacity):
        self.buffer = collections.deque(maxlen=capacity)  # 使用双端队列实现，用的时候只用一端进另一端出即可
    ...
    def sample(self, batch_size):
        ...
        return state, action, reward, next_state, done  # 返回tuple类型，不用像书上一样转为ndarray，因为tuple同样可以直接转为tensor
...
class DQN:
    ...
    def update(self, transition_dict):
        ...
        actions = torch.tensor(transition_dict['actions']).view(-1,1).to(self.device)#为了后续能够相加，使用view(-1,1)将一维tensor转为二维
