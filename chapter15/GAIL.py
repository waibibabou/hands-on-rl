...
def sample_expert_date(n_episode):
    states = []
    actions = []
...
random.seed(0)
n_episode = 1
expert_s, expert_a = sample_expert_date(n_episode)
...
class GAIL:
    ...
    def learn(self, expert_s, expert_a, agent_s, agent_a, next_s, dones):
        ...
        discriminator_loss = torch.nn.BCELoss()(agent_prob, torch.ones_like(agent_prob)) + torch.nn.BCELoss()(
        ...
        transition_dict = {'states': agent_s, 'actions': agent_a, 'rewards': rewards, 'next_states': next_s,
                           'dones': dones}
...
with tqdm(total=n_episode, desc='进度条') as pbar:
    for i in range(n_episode):
        ...
        while not done:
            action = agent.take_action(state)
            next_state, reward, done, info = env.step(action)#这里的reward不用于训练，训练的奖励值由判别器得到
        ...
        gail.learn(expert_s, expert_a, state_list, action_list, next_states_list, done_list)

        if (i + 1) % 10 == 0:
            pbar.set_postfix({'return': f'{np.mean(return_list[-10:]):.2f}'})
        pbar.update()
